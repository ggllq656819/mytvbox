...
class Spider(Spider):
    ...
    def searchContent(self, key, quick, pg=1):
        try:
            ...
            # 3. 构造多页搜索URL（适配 search.htm?search=关键词&page=页码 格式）
            search_url = f"{self.host}search.htm"
            params = {
                'search': encoded_key,
                'page': int(pg)  # 补充page参数，支持多页切换
            }
            
            # 4. 发起请求（用params自动处理URL拼接，避免格式错误）
            resp = self.fetch(
                url=search_url,
        ...
        try:
            if method.upper() == 'GET':
                resp = requests.get(
                    ...
                    params=params  # 新增params参数，支持GET请求带参数
                )
